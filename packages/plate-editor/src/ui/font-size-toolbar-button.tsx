...
export function FontSizeToolbarButton() {
  ...
  return (
    <div className="flex h-7 items-center gap-1 rounded-md bg-shark-800/60 p-0">
      ...
      <Popover open={isFocused} modal={false}>
        <PopoverTrigger asChild>
          <input
            className={cn(
              'h-full w-10 shrink-0 bg-transparent px-1 text-center text-sm hover:bg-shark-800'
            )}
            value={displayValue}
            onBlur={() => {
              setIsFocused(false);
        ...
        >
          {FONT_SIZES.map((size) => (
            <button
              key={size}
              className={cn(
                'flex h-8 w-full items-center justify-center text-sm hover:bg-shark-400 data-[highlighted=true]:bg-shark-400'
              )}
              onClick={() => {
