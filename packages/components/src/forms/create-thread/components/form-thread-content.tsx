...
export const FormThreadContent = ({ errors, control }: FormChildsProps) => {
  ...
  const handleAddImages = (
    ...
    const images = e.target.files
      ? (Array.from(e.target.files).slice(
          THREAD_IMAGES_LIMIT_DEFAULT[0],
          THREAD_IMAGES_LIMIT_DEFAULT[1],
        ) as Array<File>)
      : null;
  ...
  const contentLength =
    threadFormValues && threadFormValues.content
      ? serializeNodes(threadFormValues.content).length
      : 0;
