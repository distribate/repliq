...
export const sanitizeInput = (input: string): string => {
  ...
  const attackPattern = /<script.*?>.*?<\/script>|onerror=|onload=|SELECT.*?FROM|DROP TABLE|INSERT INTO|DELETE FROM/gi;
  sanitized = sanitized.replace(attackPattern, '');
  
  const bannedPattern = new RegExp(`(?<!\\w)(${bannedWords.join('|')})(?!\\w)`, 'giu');
