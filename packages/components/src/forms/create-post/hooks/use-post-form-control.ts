...
export const usePostFormControl = () => {
	...
	const postFormFieldsMutation = useMutation({
		mutationFn: async (
			values: CreatePostFieldType
		) => {
			qc.setQueryData(POST_FORM_FIELD_QUERY_KEY,
				(prev: CreatePostFieldType) => {
				if (!prev) return;
		...
		onSettled: async () => {
			await qc.invalidateQueries({ queryKey: POST_FORM_FIELD_QUERY_KEY })
		},
