...
export const useAuth = () => {
  ...
  const setAuthValuesMutation = useMutation({
    ...
    mutationFn: async({ values, type, }: Omit<AuthQuery, 'formState'>) => {
      ...
      const { data: originalAuthDetails } = await getUserFromOriginalAuthDetails(
        nickname,
      );
      ...
      if (!originalAuthDetails) {
        return qc.setQueryData(AUTH_QUERY_KEY, (prev: AuthQuery) => {
            return {
              ...prev,
              formState: { error: 'notFound', status: 400 }, // не найден,
            };
          },
        );
      }
      
      const { data: forumAuthDetails, error: forumAuthDetailsError } = await getUserFromForumAuthDetails(
        nickname,
      );
          ...
          if (!forumAuthDetailsError && forumAuthDetails) {
            ...
            if (error) {
              return qc.setQueryData(AUTH_QUERY_KEY, (prev: AuthQuery) => {
                return {
                  ...prev,
                  formState: { error: 'incorrectPassword', status: 400 },
                };
              });
            } else {
              // Очищаем кеш
              await qc.resetQueries({ queryKey: AUTH_QUERY_KEY });
            }
          }
        }
      }
      ...
      if (type === 'sign-up') {
        ...
        if (!forumAuthDetails) {
          ...
          if (!isCreatedSecurityCred) {
            return qc.setQueryData(AUTH_QUERY_KEY, (prev: AuthQuery) => {
                return {
                  ...prev,
                  formState: { error: 'something', status: 400 },
                };
              },
            );
          }
          ...
          if (status === 204 && isCreatedAddCred) {
            return qc.setQueryData(AUTH_QUERY_KEY, (prev: AuthQuery) => {
                return {
                  ...prev,
                  formState: { error: 'created', status: 204 },
                };
              },
            );
          }
        ...
        if (forumAuthDetails) {
          return qc.setQueryData(AUTH_QUERY_KEY, (prev: AuthQuery) => {
              return {
                ...prev,
                formState: { error: 'alreadyForum', status: 400 },
              };
            },
          );
        }
