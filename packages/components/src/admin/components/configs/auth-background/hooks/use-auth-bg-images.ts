...
export const useAuthBackgroundImage = () => {
  ...
  const addAuthImageFileMutation = useMutation({
    ...
    onSuccess: async (data, variables) => {
      if (!data)
        return toast.error("Произошла ошибка при загрузке изображения");

      if (!Array.isArray(data)) {
        if (data.error === "no-data")
          return toast.error("Что-то пошло не так при загрузке изображений.", {
            description: "Попробуйте попытку позже",
          });

        if (data.error === "limit")
          return toast.info("Изображений может быть только 50!");
      }
