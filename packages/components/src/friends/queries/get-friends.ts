  ...
  const friends = friendsData
    .map((friend) => {
      const friendNickname =
        friend.user_1 === nickname ? friend.user_2 : friend.user_1;
      ...
      const isPinned = pinnedFriends
        ? pinnedFriends.some((pinned) => pinned.recipient === friendNickname)
        : false;

      const noted = notedFriends
        ? notedFriends.find((noted) => noted.recipient === friendNickname)
        : null;
