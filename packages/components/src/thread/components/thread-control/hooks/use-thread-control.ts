...
export const useThreadControl = () => {
  ...
  const updateThreadMutation = useMutation({
    ...
    mutationFn: async(threadId: string) => {
      const newData = qc.getQueryData<ThreadControlQuery>(THREAD_CONTROL_QUERY_KEY)?.values;
    ...
    onSuccess: async(data, variables) => {
      ...
      if (data === 'no-update-fields') return toast.info('Ничего не было обновлено');
  ...
  return { updateThreadMutation, removeThreadMutation, setThreadNewValuesMutation };
};