...
export const useHistoryThreads = () => {
  ...
  const saveThread = useCallback(
    ({ nickname, title, threadId }: Omit<ThreadHistoryType, "id">) => {
  ...
  const deleteThread = ({ threadId }: Pick<ThreadHistoryType, "threadId">) => {
    if (!value) return;
  ...
  const savedThreads = value
    ? value.filter((i) => i.id === currentUser?.nickname)
    : null;
