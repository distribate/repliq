...
import { COMMENT_ACTIONS_QUERY_KEY, CommentActionsQuery } from '#thread/components/thread-comment/queries/comment-actions-query.ts';
...
export const ThreadCommentMoreActions = ({
  ...
  const handleEditContent = () => {
    return qc.setQueryData(COMMENT_ACTIONS_QUERY_KEY(commentId),
      (prev: CommentActionsQuery) =>
        ({ ...prev, isEdit: true, })
    );
  };
    ...
    <div className="absolute right-2 top-2">
      <MoreWrapper variant="small" background="transparent">
        <div className="flex flex-col gap-y-2">
          <DropdownMenuItem className="items-center gap-2" onClick={handleEditContent}>
            <Pen size={16} className="text-shark-300" />
            <Typography>Редактировать</Typography>
          </DropdownMenuItem>
          <DropdownMenuItem className="items-center gap-2" onClick={handleDeleteContent}>
