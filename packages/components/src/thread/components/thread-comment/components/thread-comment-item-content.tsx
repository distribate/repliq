import { ThreadRepliedCommentItem } from '#thread/components/thread-comment/components/thread-comment-replied-item.tsx';
import { Typography } from '@repo/ui/src/components/typography.tsx';
import { ThreadCommentProps } from '#thread/components/thread-comment/types/thread-comment-types.ts';
import { useQueryClient } from '@tanstack/react-query';
import { commentActionsQuery } from '#thread/components/thread-comment/queries/comment-actions-query.ts';
import { useControlThreadComment } from '#thread/components/thread-comment/hooks/use-control-thread-comment.ts';
import { useState } from 'react';
import AutogrowingTextarea from '@repo/ui/src/components/autogrowing-textarea.tsx';

type ThreadCommentItemContentProps = Pick<ThreadCommentProps, "content" | "replied" | "id" | "thread_id"> & {
  isOwner: boolean
}

export const ThreadCommentItemContent = ({
  content: currentContent, replied, id, isOwner, thread_id
