...
import { RemoveThreadComment, removeThreadComment } from '../queries/remove-thread-comment.ts';
...
export const useControlThreadComment = () => {
  ...
  const editCommentContentMutation = useMutation({
    ...
    onSuccess: async(data, variables) => {
      ...
      return qc.setQueryData(
        THREAD_COMMENTS_QUERY_KEY(variables.thread_id), updatedComments
      );
    },
  ...
  const deleteCommentItemMutation = useMutation({
    mutationFn: async(values: RemoveThreadComment) => removeThreadComment(values),
    onSuccess: async(data, variables) => {
