...
import { getUserActiveSessions, UserSessions } from '@repo/lib/queries/get-user-sessions.ts';
...
export const userActiveSessionsQuery = () => useQuery<
  UserActiveSessionsQuery[] | null, Error
>({
    ...
    return await Promise.all(sessions.map(async session => {
      ...
      if (session.ip) {
        if (RESTRICTED_IP.includes(session.ip)) {
          ...
          const geoData = await fetch(`http://ip-api.com/json/${session.ip}`)
