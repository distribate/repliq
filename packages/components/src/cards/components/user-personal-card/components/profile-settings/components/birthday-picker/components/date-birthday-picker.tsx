...
import {
  MonthYearPicker,
} from '#cards/components/user-personal-card/components/profile-settings/components/birthday-picker/components/month-picker.tsx';
import {
  PickerType,
} from '#cards/components/user-personal-card/components/profile-settings/components/birthday-picker/types/birthday-picker-types.ts';
import {
...
export const DateBirthdayPicker = ({
  ...
  const handleSaveBirthday = () => {
    ...
    return updateFieldMutation.mutate({
      value: parsedDate as string, field: "birthday"
    })
  }
  ...
  const onNextMonth = useCallback(() => setMonth(getNextMonth(month, max)), [ month, max ]);
  const onPrevMonth = useCallback(() => setMonth(getPreviousMonth(month, min)), [ month, min ]);
      ...
      <div className="flex flex-col gap-2 justify-center p-3 w-[360px]">
        <div className="flex items-center justify-between w-full gap-4">
          ...
          <div className="flex items-center justify-center gap-2">
            <div onClick={onPrevMonth} className="p-2 hover:bg-shark-700/20 rounded-md cursor-pointer">
              <ChevronLeftIcon size={20} className="text-shark-300" />
            </div>
            <div onClick={onNextMonth} className="p-2 hover:bg-shark-700/20 rounded-md cursor-pointer">
