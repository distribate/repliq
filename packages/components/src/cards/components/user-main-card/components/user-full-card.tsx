...
type UserCardLimitedProps = {
  title: string
}

const UserCardLimited = ({
  title
}: UserCardLimitedProps) => {
  return (
    <div>
      <Typography>
        {title}
...
export const UserFullCard = ({ nickname }: Pick<UserEntity, "nickname">) => {
  ...
  if (userStatus === 'blocked') return <UserCardLimited title="Пользователь заблокирован" />
  if (userStatus === 'private') return <UserCardLimited title="Пользователь скрыт" />
  if (userStatus === 'banned') return <UserCardLimited title="Пользователь забанен" />
        ...
        <>
          ...
          <div className="flex items-center gap-4 w-full">
            ...
            <div className="flex flex-col gap-y-2">
              ...
              <div className="w-fit">
                <UserDonate donate={donate} favoriteItemId={favoriteItem?.id ?? null} />
              </div>
            </div>
          </div>
          <Separator />
