...
import { SearchPageQuery, SearchResultsAll } from '#search/queries/search-page-query.ts';
...
export async function getSearchResults({
  type, value, limit, threadsType
}: GetSearchResults): Promise<SearchResultsAll | null> {
  const searchThreads = () => getSearchThreads({
    searchedValue: value, limit, type: threadsType ?? "title"
  });
  const searchUsers = () => getSearchUsers({
    searchedValue: value, limit
  });
