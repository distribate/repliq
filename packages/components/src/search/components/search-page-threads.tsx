...
export const SearchPageThreads = () => {
  ...
  if (!searchState.results) return <ContentNotFound title="Ничего не найдено" />;
  
  const threads = filterSearchResults<SearchThread>(searchState.results, "threads")
  ...
  return (
    <div className="flex flex-col gap-y-4 w-full h-fit">
      ...
      <div className="flex flex-col gap-y-2 w-full h-full">
        {threads.map(thread => <SearchPageThread key={thread.id} {...thread}/>)}
      </div>
    </div>
  )
}