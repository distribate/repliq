...
export const SearchPageFilter = ({
  ...
  const handleSearchFilter = (type: Pick<SearchPageFilter, 'value'>['value']) => {
  ...
  return (
    <DropdownWrapper
      properties={{ sideAlign: 'bottom', contentAlign: 'start', contentClassname: 'min-w-[180px]' }}
      trigger={
        <Button
          disabled={type === 'users'}
        ...
        <div className="flex flex-col gap-y-2 w-full h-full">
          ...
          <div className="flex flex-col gap-y-2 h-full w-full">
            {SEARCH_PAGE_FILTER.map(option => (
              ...
              >
                <Typography className={isSearchType(option.value) ? "text-caribbean-green-500" : ""}>
                  {option.title}
                </Typography>
              </DropdownMenuItem>
            ))}
