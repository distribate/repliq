  ...
  const setFriendUnNoteMutation = useMutation({
    mutationFn: async({
      ...
      return handleFriendAction(
        () => setUnNoteFriend({
          friend_id: friendId, recipient: reqUserNickname,
        }),
        'Произошла ошибка. Попробуйте позже',
        FRIENDS_QUERY_KEY(currentUser?.nickname),
      );
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: FRIENDS_QUERY_KEY(currentUser?.nickname) }),
    onError: e => { throw new Error(e.message); },
  });
  
  const setFriendNoteMutation = useMutation({
    mutationFn: async({
      reqUserNickname, friend_id, note,
    }: SetFriendNote) => {
      if (!currentUser) return;
      ...
      const isNoted = friends?.some(
        fd => fd.friend_id === friendId && !!fd.note,
      ) || false;
      
      return handleFriendAction(
        () => setNoteFriend({
          friend_id: friendId, note, isNoted, recipient: reqUserNickname,
        }),
        'Произошла ошибка. Попробуйте позже',
        FRIENDS_QUERY_KEY(currentUser?.nickname),
      );
