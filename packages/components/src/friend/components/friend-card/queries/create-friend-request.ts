...
export async function createFriendRequest(
  ...
  const userIsAcceptFriends = await validateUserFriendRequest(
    requestedUserNickname,
  );

  if (!userIsAcceptFriends)
    return {
      status: 400,
      error: "user-not-accept",
    };

  const { error, status } = await api.from("friends_requests").insert({
