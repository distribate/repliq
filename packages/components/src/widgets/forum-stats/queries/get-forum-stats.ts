...
export async function getForumStats() {
  ...
  const { count: usersForum } = await api.from("users").select("*", {
    count: "exact",
    head: true,
  });

  const { count: usersServer } = await api.from("AUTH").select("*", {
    count: "exact",
    head: true,
  });
  ...
  const { count: topicsAll } = await api.from("threads").select("*", {
    count: "exact",
    head: true,
  });
  ...
  return {
    usersRegisteredForum,
    usersRegisteredServer,
    topicsCreatedToday,
    topicsCreatedAll,
  };
}
