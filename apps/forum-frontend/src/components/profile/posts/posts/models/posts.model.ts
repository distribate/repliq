...
import { postsFilteringAtom, PostsFilteringQuery } from './filter-posts.model.ts';
import { atom, Ctx } from '@reatom/core';
import { reatomAsync, withStatusesAtom } from '@reatom/async';
import { withReset } from '@reatom/framework';
import { isParamChanged, requestedUserParamAtom } from '#components/profile/requested-user.model.ts';
...
requestedUserParamAtom.onChange((ctx, state) => isParamChanged(ctx, state, () => { 
  resetPosts(ctx); 
  consola.info("posts reset for", state) 
}))
...
export const postsAction = reatomAsync(async (ctx) => {
  ...
  onFulfill: (ctx, res) => {
    ...
    postsFilteringAtom(ctx, (state: PostsFilteringQuery) => ({ ...state, cursor: res.meta.endCursor }))
