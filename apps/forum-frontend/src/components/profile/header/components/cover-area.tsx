...
export const coverAreaVariants = cva(
  `flex flex-col lg:flex-row gap-y-4 lg:gap-y-0 justify-center w-full bg-center rounded-xl lg:mt-[4px] 
    overflow-hidden bg-cover bg-no-repeat items-center lg:justify-between px-6 lg:px-12 py-6`,
  {
    variants: {
      variant: {
...
interface CoverAreaProps extends HTMLAttributes<HTMLDivElement>, 
  VariantProps<typeof coverAreaVariants> { backgroundImage?: string; }

export const CoverArea = ({
  className, variant, backgroundColor, backgroundImage, outline, ...props
}: CoverAreaProps) => {
  return (
    <div className={coverAreaVariants({ variant, outline, className, backgroundColor })} {...props} />
  );
};
