...
interface CoverAreaProps extends HTMLAttributes<HTMLDivElement>, 
  VariantProps<typeof coverAreaVariants> { outline?: string }

export const CoverArea = ({
  className, variant, backgroundColor, outline, ...props
}: CoverAreaProps) => {
  return <div style={{ borderColor: outline }} className={coverAreaVariants({ variant, className, backgroundColor })} {...props} />
};