...
import { SidebarButton } from "#components/sidebar/desktop/components/sidebar/sidebar-button.tsx";
    ...
    <div className="flex flex-col gap-y-2 w-full">
      {SIDEBAR_TARGETS.map(({ link, title, icon: Icon }) => (
        <div key={title} className="w-full">
          {isCollapsed && (
            <Link to={link} className="flex relative w-[50px] h-[50px]">
              <SidebarButton variant={pathname === link ? "active" : "default"}>
                <Icon size={20} className="icon-color" />
              </SidebarButton>
            </Link>
          )}
          {!isCollapsed && (
            <Link to={link} className="flex relative w-full h-12">
              <SidebarButton variant={pathname === link ? "active" : "default"}>
                <Icon size={20} className="icon-color" />
