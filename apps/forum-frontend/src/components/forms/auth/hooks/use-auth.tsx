...
import { AUTH_QUERY_KEY, AUTH_TYPE_QUERY_KEY, AuthQuery } from "../queries/auth-query.ts";
import { USER_URL } from "@repo/shared/constants/routes.ts";
import { useNavigate } from "@tanstack/react-router";
import { registerForum } from '#components/forms/auth/queries/register-forum.ts';
...
export const useAuth = () => {
  ...
  const setAuthValuesMutation = useMutation({
    ...
    mutationFn: async () => {
      ...
      const authType = qc.getQueryData<"login" | "register">(AUTH_TYPE_QUERY_KEY);
      ...
      if (authType === "login") {
        const login = await loginForum({ nickname, password, token });
      ...
      if (authType === "register") {
        const register = await registerForum({ nickname, password, findout, referrer, token });
        ...
        qc.setQueryData(AUTH_QUERY_KEY, (prev: AuthQuery) => ({
          ...prev, status: REGISTER_MESSAGES[register.status],
        }));
