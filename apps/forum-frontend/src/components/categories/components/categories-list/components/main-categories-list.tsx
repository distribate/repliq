...
const MainCategoriesList = reatomComponent(({ ctx }) => {
  const data = ctx.spy(categoriesResource.dataAtom)

  if (ctx.spy(categoriesResource.statusesAtom).isPending) {
    ...
    <div className="flex flex-col gap-2 w-full">
      {data.map(category => (
        ...
        >
          ...
          <div className="flex items-center mt-2 justify-end gap-2 w-full h-full">
            <CustomLink
              to={CATEGORY_URL + category.category_id}
              ...
              <DialogContent>
                <div className="flex flex-col items-center justify-center gap-y-4 w-full">
                  ...
                  <div className="flex flex-col gap-2 p-2 w-full">
                    <Typography>
                      {category.category_description ?? "Описания пока нет"}
                    </Typography>
                  </div>
                </div>
              </DialogContent>
