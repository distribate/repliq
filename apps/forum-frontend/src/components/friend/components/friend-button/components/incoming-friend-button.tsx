...
export const IncomingFriendButton = reatomComponent<IncomingFriendButtonProps>(({ ctx, recipient, request_id, }) => {
  const isPending = ctx.spy(controlIncomingRequestAction.statusesAtom).isPending;

  const handle = (type: Parameters<typeof controlIncomingRequestAction>[1]["type"]) => {
    void spawn(ctx, async (spawnCtx) => controlIncomingRequestAction(spawnCtx, { type, request_id, recipient }))
  }
