...
import { validateAvailabilityByCurrency } from '#utils/validators/validate-availability-by-currency.ts';
...
export const createOrderRoute = new Hono()
  .post('/create-order', zValidator('json', createOrderBodySchema), async (ctx) => {
    const { paymentValue, paymentType, currency, nickname, fiatMethod, privacy } = createOrderBodySchema.parse(await ctx.req.json());
