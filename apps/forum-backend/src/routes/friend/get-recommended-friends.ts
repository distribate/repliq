...
async function getFriends(
  ...
  for (const { user_1, user_1_color, user_2, user_2_color } of rows) {
    if (user_1 !== nickname) {
      friendsMap.set(user_1, user_1_color)
    }
    if (user_2 !== nickname) {
      friendsMap.set(user_2, user_2_color)
    }
  }
  ...
  for (const { user_1, user_1_color, user_2, user_2_color } of rows) {
    if (!fullFoiSet.has(user_1)) {
      ...
      if (fromFriend && !fofMap.has(user_1)) {
        fofMap.set(user_1, { nickname: user_1, name_color: user_1_color, details: { fromFriend } });
      }
    }
    if (!fullFoiSet.has(user_2)) {
      ...
      if (fromFriend && !fofMap.has(user_2)) {
        fofMap.set(user_2, { nickname: user_2, name_color: user_2_color, details: { fromFriend } });
      }
    }
