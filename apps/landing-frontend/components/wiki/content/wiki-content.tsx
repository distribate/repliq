...
const EconomicModule = () => {
	const { setTab } = useWikiNavigation()
...
const useWikiNavigation = () => {
	...
	return { setTab, initTab, param: searchParams.get("tab") }
}
...
export const WikiContent = () => {
	const { initTab, setTab, param } = useWikiNavigation()
	const { push } = useRouter()
					...
					<TabsList className="flex flex-col p-0 rounded-xl w-full items-start">
						<Block border="mini_gray" className="gap-y-12 h-full" size="normal" rounded="big" type="column">
							...
							<div className="flex flex-col">
								...
								<Link href="/wiki/modpack" className="group">
									<Typography size="base" hover_effects="pink_drop">
										Сборки модов
									</Typography>
								</Link>
