...
const FONT_CIRCULAR = fetch(
  new URL(FONT_URLS["CIRCULAR"], import.meta.url),
).then((res) => res.arrayBuffer());
const FONT_MONO = fetch(new URL(FONT_URLS["MONO"], import.meta.url)).then(
  (res) => res.arrayBuffer(),
);
...
const getParamValue = (
  ...
  const param =
    url.searchParams.get(value) ?? url.searchParams.get(`amp;${value}`);
  const maybeDecoded = options?.decodeURI ? decodeURIComponent(param) : param;
  const maybeLowercased = options?.lowercase
    ? maybeDecoded?.toLowerCase()
    : maybeDecoded;
...
export async function handler(req: Request) {
  ...
  switch (site) {
    ...
    case "customers":
      return new ImageResponse(
        <CustomerStories title={title} customer={customer} />,
        {
          width: 1200,
          height: 630,
          fonts: [
