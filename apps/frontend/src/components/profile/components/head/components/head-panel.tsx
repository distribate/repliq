...
import { BlockUserModal } from "#components/modals/with-confirm/block-user/components/block-user-modal";
import { HeadImageUpdateModal } from "#components/profile/components/head/components/head-panel-image";
...
import { requestedUserIsSameAtom, requestedUserParamAtom, requestedUserProfileBlockedAtom } from "#components/profile/models/requested-user.model";
...
import { blockedUserAction, blockedUserDialogIsOpenAtom } from "#components/modals/with-confirm/block-user/models/blocked-user.model";
...
import { deleteBackgroundImageAction, uploadBackgroundImageAction } from "#components/profile/components/head/models/head-image.model";
...
const HeadImageDeleteModal = reatomComponent(({ ctx }) => {
...
}, "HeadImageDeleteModal")

const HeadPanelProfileSettingsModal = () => {
  return (
    <Dialog>
...
const HeadImageChangeModal = reatomComponent(({ ctx }) => {
  const isPending = ctx.spy(uploadBackgroundImageAction.statusesAtom).isPending
    || ctx.spy(deleteBackgroundImageAction.statusesAtom).isPending;
...
const UserBlockedTrigger = reatomComponent(({ ctx }) => {
  ...
  return (
    <DropdownMenuItem
      ...
      onSelect={() => blockedUserDialogIsOpenAtom(ctx, true)}
    >
...
export const HeadPanel = reatomComponent(({ ctx }) => {
