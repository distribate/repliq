...
import { requestedUserCoverImageAtom } from "#components/profile/models/requested-user.model";
...
const extendedClient = client.extend((opts) => ({
  ...opts,
  headers: { 
    ...opts.headers,
...
async function createCoverImage({ file, type, fileName }: CreateCoverImage) {
  if (type === 'custom') {
    ...
    const res = await extendedClient.post(url, { body: new Uint8Array(encoded) })
...
export const imagesLibraryAction = reatomAsync(async (ctx) => {
  return await ctx.schedule(async () => {
