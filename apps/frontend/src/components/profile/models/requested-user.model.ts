...
import { headAreaVariants } from '../components/head/components/head-area';
...
type RequestedUserCoverDetails = {
  backgroundColor: Pick<VariantProps<typeof headAreaVariants>, "backgroundColor">["backgroundColor"]
  outline: string | null
}
...
export const requestedUserProfilesIsExistAtom = atom(
  (ctx) => ctx.spy(requestedUserProfilesAtom).length >= 1, 
