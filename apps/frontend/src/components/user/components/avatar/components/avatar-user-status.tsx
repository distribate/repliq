...
const SyncUserStatus = ({ nickname }: { nickname: string }) => {
  useUpdate((ctx) => userStatusParamAtom(ctx, nickname), [nickname])
  return null;
}
      ...
      {!ctx.spy(userStatusAction.statusesAtom).isPending && (
        <TooltipProvider>
          <Tooltip delayDuration={1}>
            <TooltipTrigger className="absolute -bottom-2 -right-2 z-[2] h-4 min-h-4 max-h-4 sm:h-6 sm:min-h-6 sm:max-h-6">
              <div className="flex rounded-full bg-shark-900 p-0.5 size-6 items-center justify-center group">
