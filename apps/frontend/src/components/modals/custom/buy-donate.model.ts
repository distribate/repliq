...
import { reatomAsync, sleep, withReset, withStatusesAtom } from "@reatom/framework";
...
buyDonateModalIsOpenAtom.onChange(async (ctx, state) => {
...
export const openProcessingAction = action((ctx) => {
  const isAuthenticated = ctx.get(isAuthenticatedAtom);

  if (!isAuthenticated) {
    buyDonateModalIsOpenAtom(ctx, false)
    toast.info("Для продолжения необходимо быть зарегистрированным")
    return;
  }

  donateWidgetTypeAtom(ctx, "process")
