...
import { postViewsAction, postViewsDataAtom, postViewsMetaAtom } from "#components/post/models/post-views.model";
import { Skeleton } from "@repo/ui/src/components/skeleton.tsx";
import { UserNickname } from "#components/user/components/name/nickname";
import { Typography } from "@repo/ui/src/components/typography.tsx";
import { PostFooterViews } from "#components/post/components/post-footer/post-footer-views";
...
const ViewsExpandedList = reatomComponent(({ ctx }) => {
  ...
  return (
    <Sheet open={ctx.spy(viewsExpandedSheetIsOpenAtom)} onOpenChange={v => viewsExpandedSheetIsOpenAtom(ctx, v)}>
      <SheetContent side="right" className="flex flex-col gap-4 p-4 rounded-l-lg overflow-y-auto h-full">
