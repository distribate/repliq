...
export type CreatePostPayload = Awaited<ReturnType<typeof createPostAction>>;
...
async function createPost({
  ...
  const res = await postClient.post["create"].$post({
...
export const createPostAction = reatomAsync(async (ctx) => {
  ...
  onFulfill: (ctx, res) => {
    ...
    const newPost: PostsPayloadData[number] = {
      ...res.data,
      avatar: user.avatar,
